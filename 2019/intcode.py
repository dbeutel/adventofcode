    ...
    def run(self, stopatoutput=False):
        ...
        while True:
            ...
            if opcode == 1: # add
                ...
                a, b = [_read(self.tape, pos, next(g), self.relbase) for pos in range(self.pc + 1, self.pc + 3)]
                ...
                _write(self.tape, self.pc + 1, val, next(iter(_getmodes(modes))), self.relbase)
                self.pc += 2
            elif opcode == 4: # output
                self.output.append(_read(self.tape, self.pc + 1, next(iter(_getmodes(modes))), self.relbase))
                self.pc += 2
                if stopatoutput:
                    return 2
                ...
                self.relbase += _read(self.tape, self.pc + 1, next(iter(_getmodes(modes))), self.relbase)
                self.pc += 2
            elif opcode == 99: # halt
                return 0
